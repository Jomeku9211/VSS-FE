const Company = [
  { id: "", name: "Company" },

  { id: "NSAIL", name: "NSAIL" },
  { id: "JSW", name: "JSW" },
  { id: "Manoj", name: "Manoj" },
  { id: "AMNS", name: "AMNS" },
  { id: "TATA", name: "TATA" },
  { id: "ESSAR", name: "ESSAR" },
  { id: "SAIL", name: "SAIL" },
  { id: "ASIAN INVETORY", name: "ASIAN INVENTORY" },
  
];
const Product = [
  { id: "", name: "Product" },
  { id: "GPC", name: "GPC" },
  { id: "GPS", name: "GPS" },
  {id:"Hr Sheet" , name:"Hr Sheet"},
  {id:"HR Coil" , name:"HR Coil"},
  { id: "GP Coil", name: "GP Coil" },
  {id:"CR Sheet" , name:"CR Sheet"},
  {id:"CR coil" , name:"CR coil"},
  { id: "GC Coil", name: "GC Coil" },
  { id: "HR", name: "HR" },
  { id: "CR", name: "CR" },
  { id: "Color Coil", name: "Color Coil" },
  { id: "Profile Sheet", name: "Profile Sheet" },
  { id: "Acce.", name: "Acce." },
  { id: "GC Sheet", name: "GC Sheet" },
  {id:"GP Sheet",name:"GP Sheet"},

];
const Grade = [
  { id: "", name: "Grade" },
  { id: "PRIME", name: "PRIME" },
  { id: "SECOND", name: "SECOND" },
  { id: "DEFECTIVE", name: "DEFECTIVE" },
  { id: "TEST", name: "TEST" },
];
const Color = [
  { id: "", name: "Color" },
  { id: "REG", name: "REG" },
  { id: "SP", name: "SP" },
  { id: "TL", name: "TL" },
  { id: "BLUE", name: "BLUE" },
  { id: "WHITE", name: "WHITE" },
  { id: "SECO RED", name: "SECO RED" },
  { id: "BRICK RED", name: "BRICK RED" },
  { id: "YEllOW", name: "YEllOW" },
  { id: "DARK GREY", name: "DARK GREY" },
  { id: "LIGHT GREY", name: "LIGHT GREY" },
  { id: "ENVIR GREEN", name: "ENVIR GREEN" },
  { id: "ROYAL BLUE", name: "ROYAL BLUE" },
];

const Coating = [
  { id: "", name: "Coating" },
  { id: "70", name: "70" },
  { id: "100", name: "100" },
  { id: "150", name: "150" },
  { id: "80", name: "80" },
  { id: "90", name: "90" },
  { id: "120", name: "120" },
  { id: "180", name: "180" },
];

const Temper = [
  { id: "", name: "Temper" },
  { id: "Full Hard", name: "Full Hard" },
  { id: "Semi Hard", name: "Semi Hard" },
  { id: "Soft", name: "Soft" },
  { id: "Extra Soft", name: "Extra Soft" },
  { id: "DD", name: "DD" },
  { id: "EDD", name: "EDD" },
  { id: "2062", name: "2062" },
  { id: "1079", name: "1079" },
];
const Guard = [
  { id: "", name: "Guard" },
  { id: "Without", name: "Without" },
  { id: "SkyLine", name: "SkyLine" },
  { id: "Hindustan", name: "Hindustan" },
  { id: "Asia", name: "Asia" },
  { id: "Rangoli", name: "Rangoli" },
];

const constants = { Company, Temper, Coating, Product, Color, Grade, Guard };
export default constants;
